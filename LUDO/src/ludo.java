import java.awt.*;
import java.awt.event.*;
import java.util.Random;
import javax.swing.*;

public class ludo extends JFrame 
{ 
     int temp1,temp2,temp3,temp4;                             // to store current position 
     int x=0,y=0,WIDTH=30,HEIGHT=30,flag=0,dice;
     boolean player1=true,player2=false,player3=false,player4=false;  //to find whose turn is currently on alive
     boolean isalive1=false,isalive2=false,isalive3=false,isalive4=false;   //run one time when player got 1 first time
     boolean finish1=false,finish2=false,finish3=false,finish4=false;      //to see who hase reached at his goal
     boolean iswin=false;
     Random r=new Random();
     JTextField d=new JTextField();
     JTextField t [] =new JTextField[73];
     JTextField p1=new JTextField("O1");
     JTextField p2=new JTextField("O2");
     JTextField p3=new JTextField("O3");
     JTextField p4=new JTextField("O4");
     JButton b=new JButton("DICE");
     JButton restart=new JButton("RESTART");
     JLabel l1 = new JLabel("Ludo");
     JLabel winner=new JLabel();
     Font f;
     JLabel masthead=new JLabel("Rishabh Sharma:Founder & CEO");
     public ludo()
     {       
             
         l1.setBounds(550, 50, 150, 100);
         masthead.setBounds(5, 550, 1000, 20);
         winner.setBounds(520, 400, 300, 150);
         restart.setBounds(550, 390, 130, 50);
         //decorations start............
         setTitle("Ludo");
         winner.setFont(new Font("Arial",Font.BOLD,32));
         l1.setFont(new Font("Arial",Font.BOLD,60));
         b.setFont(new Font("Arial",Font.BOLD,20));
         p1.setFont(new Font("Arial",Font.BOLD,20));
         p2.setFont(new Font("Arial",Font.BOLD,20));
         p3.setFont(new Font("Arial",Font.BOLD,20));
         p4.setFont(new Font("Arial",Font.BOLD,20));
         p1.setMargin(new Insets(60,70,50,50));
         p2.setMargin(new Insets(60,70,50,50));
         p3.setMargin(new Insets(60,70,50,50));
         p4.setMargin(new Insets(60,70,50,50));
         p1.setBackground(new Color(127,227,127));          //Green
         p2.setBackground(new Color(0,0,255));          // blue
         p3.setBackground(new Color(255,255,0));           //yellow
         p4.setBackground(new Color(255,0,0));             //red
         b.setBackground(new Color(100,200,250));           //Blue
         d.setMargin(new Insets(0,15,0,0));
         d.setFont(new Font("Arial",Font.BOLD,30));
         masthead.setFont(new Font("SansSerif",Font.BOLD,10));
         restart.setFont(new Font("Arial",Font.BOLD,20));
         restart.setBackground(new Color(100,200,250));
         
         //decorations end.........
         add(l1);
         add(masthead);
         add(restart);
         add(b);
         add(d);
         add(p1);
         add(p2);
         add(p3);
         add(p4);
         add(winner);
         this.setLayout(null);
         setVisible(true);
         setSize(800,600); 
         setResizable(false);
         setDefaultCloseOperation(3);
         
         for(int i=0;i<73;i++)
         {
           t[i]=new JTextField(); 
           add(t[i]);
           t[i].setMargin(new Insets(0,5,0,0));
           t[i].setFont(new Font("Arial",Font.BOLD,14));
           t[i].setBackground(new Color(255,255,227));                //Brown light
         }
         for(int i=0;i<73;i++)              //starting of for loop for setBounds of blocks
         {
          if(i<=5)
          {
             x+=30;
             y=210;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
          }
          if(i>=6&&i<=11)
          {
             x=210;
             y-=30;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
          }
          if(i==12)
          {
             x=240;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
             y-=30;
          }
          if(i<=18&&i>=13)
          {
             x=270;
             y+=30;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
          }
          if(i>=19&&i<=24)
          {
             x+=30;
             y=210;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
          }
          if(i==25)
          {
             x=450;
             y+=30;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
             x+=30;
          }
          if(i<=31&&i>=26)
          {
             x-=30;
             y=270;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
          }
          if(i<=37&&i>=32)
          {
             x=270;
             y+=30;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
          }
          if(i==38)
          {
             x-=30;
             y=450;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
             y+=30;
          }
          if(i>=39&&i<=44)
          {
             x=210;
             y-=30;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
          }
          if(i>=45&&i<=50)
          {
             x-=30;
             y=270;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
          }
          if(i==51)
          {
             y-=30;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
          }
          if(i>=52&&i<=56)
          {
             x+=30;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
             t[i].setBackground(new Color(127,227,127));          //Green
          }
         if(i==57)
             continue;
         if(i==58){
             x=240;
             y=60;
             t[58].setBounds(x, y, WIDTH, HEIGHT);
             t[58].setBackground(new Color(0,0,255));          // blue
          }
         if(i>=59&&i<=62){
             y+=30;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
             t[i].setBackground(new Color(0,0,255));          // blue
         }
         if(i==63){
             x=420;y=240;
             t[63].setBounds(x, y, WIDTH, HEIGHT);
             t[63].setBackground(new Color(255,255,0));           //yellow
         }
         if(i>=64&&i<=67){
             x-=30;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
             t[i].setBackground(new Color(255,255,0));           //yellow
         }
         if(i==68){
             x=240;y=420;
             t[68].setBounds(x, y, WIDTH, HEIGHT);
             t[68].setBackground(new Color(255,0,0));             //red
         }
         if(i>=69&&i<=72){
             y-=30;
             t[i].setBounds(x, y, WIDTH, HEIGHT);
             t[i].setBackground(new Color(255,0,0));             //red
         }
        }                                                //end of for loop for setBounds
      t[57].setBounds(210, 210, 90, 90);      //the final block or center block  
      t[1].setBackground(new Color(127,227,127));          //Green
      t[14].setBackground(new Color(0,0,255));          // blue
      t[27].setBackground(new Color(255,255,0));           //yellow
      t[40].setBackground(new Color(255,0,0));             //red
      b.setBounds(550, 190, 100, 50);         //click to play      
      d.setBounds(550, 290, 50, 50);          //random variable textfield            
      p1.setBounds(30, 30, 180, 180);         
      p2.setBounds(300, 30, 180, 180);
      p3.setBounds(300, 300, 180, 180);
      p4.setBounds(30, 300, 180, 180);
      t[57].setFont(new Font("Arial",Font.BOLD,30));
      t[57].setText("KING");
      t[57].setBackground(new Color(255,0,255));                //Yellow
      d.setBackground(new Color(100,200,250));
         
      restart.addActionListener(new ActionListener()
                 {
                        public void actionPerformed(ActionEvent e){
                            
                            player1=true;player2=false;player3=false;player4=false;
                            finish1=false;finish2=false;finish3=false;finish4=false;
                            if(isalive1)
                            t[temp1].setText(" ");
                            if(isalive2)
                            t[temp2].setText(" ");
                            if(isalive3)
                            t[temp3].setText(" ");
                            if(isalive4)
                            t[temp4].setText(" ");
                            dead(1);
                            dead(2);
                            dead(3);
                            dead(4);
                            winner.setText(" ");
                            for(int i=0;i<73;i++){
                                if(i==57)
                                    continue;
                                t[i].setBackground(new Color(255,255,227));
                            }
                            t[1].setBackground(new Color(127,227,127));          //Green
                            t[14].setBackground(new Color(0,0,255));          // blue
                            t[27].setBackground(new Color(255,255,0));           //yellow
                            t[40].setBackground(new Color(255,0,0));             //red   
                            for (int i = 52; i < 57; i++) {
                                t[i].setBackground(new Color(127,227,127));          //Green
                            }
                            for (int i = 58; i < 63; i++) {
                                t[i].setBackground(new Color(0,0,255));          // blue
                            }
                            for (int i = 63; i < 68; i++) {
                                t[i].setBackground(new Color(255,255,0));          // yellow
                            }
                            for (int i = 68; i < 73; i++) {
                                t[i].setBackground(new Color(255,0,0));             //red
                            }
                        }    
                    });
      
      b.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{ 
                            dice=1+r.nextInt(6);
                            d.setText(String.valueOf(dice));
	                    if(player1){
                                turn1();
                                player1=false;
                                player2=true;
                                if(finish1)
                                    winner("O1");
                                if((!finish1)&&(dice==6)&&isalive1){
                                    player1=true;
                                    player2=false;
                                }
                                return;
                            } 
                            if(player2){
                                turn2();
                                player2=false;
                                player3=true;
                                if(finish2)
                                    winner("O2");
                                if((!finish2)&&(dice==6)&&isalive2){
                                    player2=true;
                                    player3=false;
                                }                                
                                return;
                            } 
                            if(player3){
                                turn3();
                                player3=false;
                                player4=true;
                                if(finish3)
                                    winner("O3");
                                if((!finish3)&&(dice==6)&&isalive3){
                                    player3=true;
                                    player4=false;
                                }                               
                                return;
                            } 
                            if(player4){
                                turn4();
                                player4=false;
                                player1=true;
                                if(finish4)
                                    winner("O4");
                                if((!finish4)&&(dice==6)&&isalive4){
                                    player4=true;
                                    player1=false;
                                }                                
                                return;
                            } 
                        }
		});
      
}
     public void turn1(){
             if(temp1<57&&isalive1){
                t[temp1].setText(" ");
                int get=dice;  
                flag=temp1;
                temp1+=get;
                if(temp1==57){
                    finish1=true;
                }
                if(temp1>57)
                temp1=flag;
                t[temp1].setText("O1");
                if(finish1){
                     t[temp1].setMargin(new Insets(0,20,0,0));
                }
            } 
             if(dice==1&&!isalive1){
                 temp1=1;
                 t[temp1].setText("O1");
                 p1.setText(" ");
                 isalive1=true;
            }
            if(temp1==temp2&&!finish1)
                dead(2);
            if(temp1==temp3&&!finish1)
                dead(3);
            if(temp1==temp4&&!finish1)
                dead(4);
     }
     public void turn2(){
         if(isalive2){
         if(temp2>=14&&temp2<=51){
                 t[temp2].setText(" ");
                 int get=dice; 
                 temp2+=get;
                 if(temp2==52)
                     temp2=0;
                 if(temp2==53)
                     temp2=1;
                 if(temp2==54)
                     temp2=2;
                 if(temp2==55)
                     temp2=3;
                 if(temp2==56)
                     temp2=4;
                 if(temp2==57)
                     temp2=5;
                 t[temp2].setText("O2");
         }
         else if(temp2>=0&&temp2<=12){
                 t[temp2].setText(" ");
                 int get=dice;  
                 temp2+=get;
                 if(temp2==13)
                     temp2=58;
                 if(temp2==14)
                     temp2=59;
                 if(temp2==15)
                     temp2=60;
                 if(temp2==16)
                     temp2=61;
                 if(temp2==17)
                     temp2=62;
                 if(temp2==18)
                     temp2=57;
                 t[temp2].setText("O2");
         }
         else if(temp2>=58&&temp2<=62){
                 t[temp2].setText(" ");
                 int get=dice; 
                 flag=temp2;
                 temp2+=get;
                 if(temp2==63){
                     temp2=57;
                     finish2=true;
                 }
                 if(temp2>62)
                     temp2=flag;
                 t[temp2].setText("O2");
                 if(finish2){
                       //t[temp2].setBackground(new Color(0,0,255));
                       t[temp2].setMargin(new Insets(0,20,0,0));
                }
         }
        }
         if(dice==1&&!isalive2){
                 temp2=14;
                 t[temp2].setText("O2");
                 p2.setText(" ");
                 isalive2=true;
             }
         if(temp2==temp1&&!finish2)
                dead(1);
         if(temp2==temp3&&!finish2)
                dead(3);
         if(temp2==temp4&&!finish2)
                dead(4);
     }
     public void turn3(){
         if(isalive3){
         if(temp3>=27&&temp3<=51){
                 t[temp3].setText(" ");
                 int get=dice;  
                 temp3+=get;
                 if(temp3==52)
                     temp3=0;
                 if(temp3==53)
                     temp3=1;
                 if(temp3==54)
                     temp3=2;
                 if(temp3==55)
                     temp3=3;
                 if(temp3==56)
                     temp3=4;
                 if(temp3==57)
                     temp3=5;
                 t[temp3].setText("O3");
         }
         else if(temp3>=0&&temp3<=25){
                 t[temp3].setText(" ");
                 int get=dice;  
                 temp3+=get;
                 if(temp3==26)
                     temp3=63;
                 if(temp3==27)
                     temp3=64;
                 if(temp3==28)
                     temp3=65;
                 if(temp3==29)
                     temp3=66;
                 if(temp3==30)
                     temp3=67;
                 if(temp3==31){
                     temp3=57;
                     finish3=true;
                 }
                 t[temp3].setText("O3");
                 if(finish3){
                  //   t[temp3].setBackground(new Color(255,255,0));
                     t[temp3].setMargin(new Insets(0,20,0,0));
                }
         }
         else if(temp3>=63&&temp3<=67){
                 t[temp3].setText(" ");
                 int get=dice;  
                 flag=temp3;
                 temp3+=get;
                 if(temp3==68){
                     temp3=57;
                     finish3=true;
                 }
                 if(temp3>67)
                     temp3=flag;
                 t[temp3].setText("O3");
                 if(finish3){
                   //t[temp3].setBackground(new Color(255,255,0));
                     t[temp3].setMargin(new Insets(0,20,0,0));
                }
         }
        }
         if(dice==1&&!isalive3){
                 temp3=27;
                 t[temp3].setText("O3");
                 p3.setText(" ");
                 isalive3=true;
             }
         if(temp3==temp1&&!finish3)
                dead(1);
         if(temp3==temp2&&!finish3)
                dead(2);
         if(temp3==temp4&&!finish3)
                dead(4);
     }
     public void turn4(){
         if(isalive4){
         if(temp4>=40&&temp4<=51){
                 t[temp4].setText(" ");
                 int get=dice; 
                 temp4+=get;
                 if(temp4==52)
                     temp4=0;
                 if(temp4==53)
                     temp4=1;
                 if(temp4==54)
                     temp4=2;
                 if(temp4==55)
                     temp4=3;
                 if(temp4==56)
                     temp4=4;
                 if(temp4==57)
                     temp4=5;
                 t[temp4].setText("O4");
         }
         else if(temp4>=0&&temp4<=38){
                 t[temp4].setText(" ");
                 int get=dice; 
                 temp4+=get;
                 if(temp4==39)
                     temp4=68;
                 if(temp4==40)
                     temp4=69;
                 if(temp4==41)
                     temp4=70;
                 if(temp4==42)
                     temp4=71;
                 if(temp4==43)
                     temp4=72;
                 if(temp4==44)
                     temp4=57;
                 t[temp4].setText("O4");
         }
         else if(temp4>=68&&temp4<=72){
                 t[temp4].setText(" ");
                 int get=dice;  
                 flag=temp4;
                 temp4+=get;
                 if(temp4==73){
                     temp4=57;
                     finish4=true;
                 }
                 if(temp4>72)
                     temp4=flag;
                 t[temp4].setText("O4");
                 if(finish4){
               //      t[temp4].setBackground(new Color(255,255,0));
                     t[temp4].setMargin(new Insets(0,20,0,0));
                }
         }
        } 
         if(dice==1&&!isalive4){
                 temp4=40;
                 t[temp4].setText("O4");
                 p4.setText(" ");
                 isalive4=true;
             }
         if(temp4==temp1&&!finish4)
                dead(1);
         if(temp4==temp2&&!finish4)
                dead(2);
         if(temp4==temp3&&!finish4)
                dead(3);
     }
     public void dead(int b){
         if(b==1){
             isalive1=false;
             p1.setText("O1");
             temp1=100;
         }
         if(b==2){
             isalive2=false;
             p2.setText("O2");
             temp2=100;
         }
         if(b==3){
             isalive3=false;
             p3.setText("O3");
             temp3=100;
         }
         if(b==4){
             isalive4=false;
             p4.setText("O4");
             temp4=100;
         }
     }
    public void winner(String a){
        if(!iswin){
            winner.setText("Hurray "+a+" is King");
            iswin=true;
        }
    }
	public static void main(String[] args) { 
		ludo l=new ludo();

	}
}